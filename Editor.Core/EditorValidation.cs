            ...
            switch (placement.Kind)
            {
                ...
                case EditorPlacementKind.PlayerSpawn:
                    ...
                    if (placement.Team <= 0)
                        errors.Add($"Placement {placement.Id} must use a positive team");
                    break;
                case EditorPlacementKind.ConquestFlag:
                    if (placement.ArchetypeId != "demiurge:conquest-flag")
                        errors.Add($"Placement {placement.Id} has unknown conquest flag {placement.ArchetypeId}");
                    if (placement.Team != 0)
