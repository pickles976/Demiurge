...
public static class EditorTargeting
{
    ...
    /// <summary>
    /// The solid and air SDF samples bracketing a rendered surface. Block stamps are centered on
    /// samples, unlike object anchors which occupy floor-based cells.
    /// </summary>
    public static EditorTargetCells Samples(Vector3 hitPoint, Vector3 normal)
    {
        if (!IsFinite(hitPoint) || !IsFinite(normal) || normal.LengthSquared() < 1e-8f)
            throw new ArgumentException("Target hit and normal must be finite and non-zero");
    ...
    private static Int3 SampleAt(Vector3 point)
        => new((int)MathF.Round(point.X), (int)MathF.Round(point.Y), (int)MathF.Round(point.Z));
